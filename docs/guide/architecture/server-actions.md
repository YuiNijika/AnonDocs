...
- `statusCode()` 控制 HTTP 状态码，也就是响应里的数字型 `code`
- `code()` 控制成功语义里的 `business_code`
...
use Anon\Core\Action\Action;
use Anon\Http\Requests\PublishPostRequest;
...
如果你希望更贴近 REST API 契约，还可以直接用 helper：
...
如果你想在发布前快速检查 Action 的文档元信息，也可以直接跑：
